...
class JwtCsrfFilter(private val tokenRepository: CsrfTokenRepository,
                    private val resolver: HandlerExceptionResolver) : OncePerRequestFilter() {
    ...
    override fun doFilterInternal(
        ...
        if (request.servletPath == "/auth/login") {
            ...
            var actualToken: String = request.getHeader(csrfToken.headerName)
            ...
            try {
                if (actualToken.isNotEmpty()) {
                    Jwts.parser()
                        .setSigningKey((tokenRepository as JwtTokenRepository).getSecret())
